...
import { Menu, Search, ChevronDown, ChevronRight, ChevronLeft } from "lucide-react";
...
export function Sidebar({ isCollapsed, setIsCollapsed }: SidebarProps) {
    ...
    return (
        <motion.aside
            ...
            className="fixed left-0 top-0 z-40 h-screen flex flex-col text-[var(--text-secondary)]"
        >
            ...
            >
                {/* Header / Logo */}
                <div className="flex items-center justify-between h-20 px-5 border-b border-[var(--border-subtle)] shrink-0">
                    <AnimatePresence mode="wait">
                        {!isActuallyCollapsed && (
                            ...
                            >
                                ...
                                <span className="text-[var(--text-primary)] font-bold text-lg tracking-tight">The Mirrow</span>
                            </motion.div>
                        )}
                    </AnimatePresence>
                        ...
                        return (
                            <div key={item.title}>
                                ...
                                {hasSubmenu ? (
                                    ...
                                    `}
                                    >
                                        <div className={`flex items-center gap-3 ${isActuallyCollapsed ? 'mx-auto' : ''}`}>
                                            {Icon && <Icon className={`w-5 h-5 ${isOpen ? 'text-[var(--text-primary)]' : 'text-[var(--text-muted)] group-hover:text-[var(--text-secondary)]'}`} />}
                                            {!isActuallyCollapsed && <span>{item.title}</span>}
                                        </div>
                                        {!isActuallyCollapsed && (
                                            <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${isOpen ? 'rotate-180 text-[var(--text-primary)]' : 'text-[var(--text-muted)]'}`} />
                                        )}
                                    </button>
                                ) : (
                                    <Link
                                    ...
                                    `}
                                    >
                                        {Icon && <Icon className={`w-5 h-5 ${isActive ? 'text-[var(--text-primary)]' : 'text-[var(--text-muted)] group-hover:text-[var(--text-secondary)]'}`} />}
                                        {!isActuallyCollapsed && <span>{item.title}</span>}
                                    </Link>
                                )}
                    ...
                    {!isActuallyCollapsed ? (
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-semibold text-[var(--text-primary)] truncate">GiovanyDev</p>
            ...
            >
                {isActuallyCollapsed ? <ChevronRight className="w-3 h-3" /> : <ChevronLeft className="w-3 h-3" />}
            </button>
